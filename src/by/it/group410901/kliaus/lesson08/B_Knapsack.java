...
public class B_Knapsack {
    ...
    int getMaxWeight(InputStream stream ) {
        ...
        for (int i = 0; i < n; i++) { // перебираем каждый слиток
            // Проходим по рюкзаку в обратном порядке, чтобы не использовать слиток более одного раза
            for (int j = w; j >= gold[i]; j--) {
                dp[j] = Math.max(dp[j], dp[j - gold[i]] + gold[i]); // обновляем максимум
            }
        }

        int result = dp[w]; // максимальный вес для полного рюкзака
        //!!!!!!!!!!!!!!!!!!!!!!!!!     КОНЕЦ ЗАДАЧИ     !!!!!!!!!!!!!!!!!!!!!!!!!
        return result;
    }
