package by.it.group451004.kiselev.lesson05;
...
public class A_QSort {
    ...
    private class Segment implements Comparable<Segment> {
        int start; //отрезок времени старт стоп
        // сортировка по началу если равны по концу
        int stop; //m событий и n отрезков

        Segment(int start, int stop) {
            if (start < stop) { //использование медианы из трёх элементов для выбора опорного
                this.start = start;
                this.stop = stop;
            } else {
                this.stop = start;
