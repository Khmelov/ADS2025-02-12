package by.it.group451002.troshin.lesson10;
...
public class MyPriorityQueue<E> implements Queue<E> {
    ...
    public boolean contains(Object o) {
        for (int i = 0; i < size; i++) {
            if (Objects.equals(o, heap[i])) return true;
        }
        return false;
    }
    ...
    public boolean removeAll(Collection<?> c) {
        boolean modified = false;
        // Используем подход с перестроением кучи
        @SuppressWarnings("unchecked")
        ...
        if (modified) {
            // Восстанавливаем кучу вручную (heapify)
    ...
    private void siftUp(int index) {
        ...
        while (index > 0) {
            ...
            E parentElem = heap[parent];
            if (compare(element, parentElem) >= 0) break;
    ...
    private void siftDown(int index) {
        ...
        while (index < half) {
            int child = (index << 1) + 1;
            E childElem = heap[child];
            int right = child + 1;
            if (right < size && compare(heap[right], childElem) < 0) {
                child = right;
                childElem = heap[right];
            }
            if (compare(element, childElem) <= 0) break;
            heap[index] = childElem;
            index = child;
        }
        heap[index] = element;
    }
