package by.it.group451002.shutko.lesson12;

import java.util.Collection;
import java.util.Map;
import java.util.Set;

/*Создать собственную реализацию интерфейса Map<Integer, String>
на основе АВЛ-дерева (самобалансирующегося бинарного дерева поиска)
без использования стандартных коллекций Java.
 */
public class MyAvlMap implements Map<Integer, String> {
    // реализует упрощённый Map, использующий сбалансированное дерево AVL для хранения пар "ключ-значение".
    private class Node {  // класс, представляющий узел дерева, где каждый узел хранит
        ...
        // Правый поворот (когда левое поддерево слишком высокое)
        ...
        // Левый поворот (когда правое поддерево слишком высокое)
    ...
    private Node recGet(Node n, Integer key) {  // Рекурсивно ищет узел с заданным ключом
        if (n == null)
            return null;
        // Если ключ меньше текущего - идём влево
        // Если ключ больше текущего - идём вправо
        // Если равен - нашли
        else if (key < n.key)
    ...
    private Node recRemove(Node n, Object key) {
        ...
        // Если у узла есть оба потомка, находим минимальный элемент в правом поддереве
            ...
            if (r == null)
                return l;
            Node min = r.findMin();
            // Заменяем удаляемый узел на найденный минимальный
    ...
    private void toStr(Node n, StringBuilder res) {
        ...
        if (n == null)
            return;
        // Используется инфиксный обход, который для выводит элементы в отсортированном порядке по ключам.
        toStr(n.left, res);
