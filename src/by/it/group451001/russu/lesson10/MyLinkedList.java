...
public class MyLinkedList<E> implements Deque<E> {
    public class MyNode<E> {
        public E v;
        public Object prev = null, next = null;
        public MyNode(E v){ this.v=v; }
    }
    ...
    public E remove(int index){
        ...
        if(index < (size>>1)){
            cur = head;
            for(int i=0;i<index;i++) cur = (MyNode<E>)cur.next;
        } else {
            cur = tail;
            for(int i=size-1;i>index;i--) cur = (MyNode<E>)cur.prev;
        }
        E val = cur.v;
        unlink(cur);
        return val;
    ...
    public void addLast(E element){
        if(element==null) throw new NullPointerException();
        MyNode<E> n = new MyNode<E>(element);
        if(size==0){
            head = tail = n;
        } else {
            tail.next = n;
    ...
    private void unlink(MyNode<E> n){
        MyNode<E> p = (MyNode<E>)n.prev;
        MyNode<E> q = (MyNode<E>)n.next;
