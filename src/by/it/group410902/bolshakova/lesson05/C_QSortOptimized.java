package by.it.group410902.bolshakova.lesson05;
...
public class C_QSortOptimized {
    ...
    int[] getAccessory2(InputStream stream) throws FileNotFoundException {
        ...
        for (int i = 0; i < m; i++) {
            int x = points[i];
            int upper = upperBoundStart(segments, x);
            if (upper == 0) {
                result[i] = 0;
                continue;
            }
            int count = lowerBoundStop(segments, x, 0, upper);
            result[i] = count;
        }
    ...
    private int upperBoundStart(Segment[] segments, int x) {
        int left = 0;
    ...
    private int lowerBoundStop(Segment[] segments, int x, int low, int high) {
        int left = low;
        int right = high;
        while (left < right) {
            int mid = (left + right) / 2;
            if (segments[mid].stop < x) {
                right = mid;
            } else {
                left = mid + 1;
            }
